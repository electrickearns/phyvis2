...
This pacakge is composed of two functions: 
    ...
    The second, DNA.vis, takes a fasta file of a single sequence and produces the frequencies of each nucleotide, the GC count, the lenth of the sequences in addition to a plot showing the relative  
...
The following code uses {ape} to gather further information: base.freq calculates the frequency of the four nucleotides in the FASTA file. This can be useful for verifying normal expected ranges. 
...
##Trim ends is useful when one needs to trim their data. In out case, our data was already trimmed, so this function was not needed in this case. One needs either the quality of the code or the primer information to trim ends of DNA. 
...
There are various ways to represent trees. Trees can also portray different unformation such as having a rooted or unrooted tree.
...
The followinf code creates the heatmap!
